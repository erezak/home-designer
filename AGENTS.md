...
- **View types and rendering**: The app supports three view types: 'elevation' (front view showing width×height), 'plan' (top-down showing width×depth), and '3d' (isometric showing all three dimensions). When adding view-specific logic, always handle all three cases. The 3D view uses CSS box-shadow for depth visualization and skewed divs for isometric side/top faces.